...
export interface AppointmentCancellationApplication {
  cancellation(appointmentId: string, reason: string): Promise<boolean>;
}
export class AppointmentCancellationApplicationImpl
  implements AppointmentCancellationApplication
{
  constructor(
  ...
  async cancellation(appointmentId: string, reason: string): Promise<boolean> {
    const result =
      await this.appointmentCancellationDataAccess.cancelAppointment(
        appointmentId,
        reason
      );
    return result;
